...
Для локальной разработки (без Docker) или переопределения параметров создайте файл `.env`:

```bash
cp .env.example .env
```

Затем отредактируйте `.env` под ваши нужды. Система автоматически подхватит эти значения при запуске.
...
Основные настройки хранятся в `docker-compose.yml` и `.env`. Полный список доступных переменных с примерами значений можно найти в файле [.env.example](.env.example).
Ключевые параметры:
